...
function minimizeAllVisibleNotAllowedWindows{
    write-host "entering minimizeNotAllVisibleNotAllowedWindows" -ForegroundColor Red
...
function minimizeAllVisibleSuspectWindows{
    write-host "entering minimizeAllVisibleSuspectWindows" -ForegroundColor Red
    $visibleProceses = Get-Process | Where-Object { $_.MainWindowHandle -ne 0 }
    logError("starting to check for visible windows to be minimized * * * * * * * * * * * * * * * * * *")
    foreach ($p in $visibleProceses) {
        $title = $p.MainWindowTitle.trim() 
    ...
    write-host "minimizing chrome processes" -ForegroundColor Red
    $chromeProcesses = Get-Process | where-object { $_.processname -like "chrome*" }
    foreach ($p in $chromeProcesses) {
        $title = $p.MainWindowTitle.trim() 
        $procName = $p.ProcessName
        write-host "*****************", $procName, $title
        logError("minimizing chrome window: $procName+$title ($($p.ID))")
        Set-WindowStyle $p 'MAXIMIZE'
    }
    ...
    while ($true) {
        ...
        try {
            ...
            if ($title -ne "") {
                ...
                $cpu = 0
                $deltaCpu = 0
                # if title is "", then there is no normal window on top of the windows stack, which might mean 
                # some trick is used to have that windows visible (like youtube of VLC) but not appearing as the top visible window; 
                # it might also mean there is a right-click kind-of menu that is activated, which prevents the 
                # monitoring system to detect normally the visible window
                # so in that case, just to be sure, let's minimize all the windows that are visible (but not necessarily on top) and
                # which contain a blacklisted keyword
                minimizeAllVisibleSuspectWindows
            }      
