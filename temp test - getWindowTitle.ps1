...
PS> & 'C:\Users\derruer\mydata\projects\powershell\temp test - getWindowTitle.ps1'
...
#>
    ...
    while ($true) {
        ...
        try {
            ...
            if ($iterationNb -eq 0) {
                debug ("debug : $debug")
                write-host "start test wrong webserver !!!!!!!!!!!!!!!!!!!!!!!!!"
                ...
                write-host $myMsg
                $res = Invoke-RestMethod -Uri $url
                write-host "after invoke-restmethod !!!!!!!!!!!!!!!!!!!!!!!!!"
                $keywords = $res.keywords
                $errMsg = $res.errMsg
                $myMsg = "$($datetime) - keywords found in DB : ->" + $keywords + "<- errMsg : " + $errMsg 
                debug $myMsg
 
                $keywords = @("io game", "Petridish", "YouTube", "Mitos\.is")
                $myMsg = "$($datetime) - keywords found in param file : ->" + $keywords + "<- errMsg : " + $errMsg 
                #$myMsg | out-file -append -filepath $errorFile
                debug("test1")
                debug $myMsg
                debug("test2")
